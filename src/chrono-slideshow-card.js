...
//          mismatched — CSS Grid auto-sizes each row to its tallest cell, so
//          the height difference was a direct visual consequence, not a bug.
//          Settled on 40px for both, scoped to this panel only via
//          `.zone-band-grid chrono-cs-select::part(combobox)` — required
//          adding part="combobox" to CsSelect's own internal div (a CSS
//          Shadow Part), since it previously had no external sizing hook and
//          its height is otherwise hardcoded globally. Every other CsSelect
//          instance elsewhere in the editor is untouched, still 56px.
